...
| 4 — UI kit playground | [`qcobjects-ui-kit`](../qcobjects-ui-kit) | Reusable widget kit (card, counter, tabset, composed panel with nested components) styled with **Tailwind CSS scoped into shadow roots** and with **widget-owned interaction** via the component `done()`/`hostElements` hooks; plus a live shadow-DOM inspector. Files upstream blocker `QuickCorp/QCObjects#120`. | ✅ |
